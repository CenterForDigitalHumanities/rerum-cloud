...
reliably and the changes between them become an interesting piece of the academic
...
## Forks: the family tree of digital objects
...
`rr:unpublished` or `rr:published`. Not having either of these Motivations means nothing—it
...
The `rr:unpublished` Motivation is simple. The statement is just that the content of the
annotation (or object) is not intended for publication—it is shared in good faith for use
in discovery or as a seed for further work only. An `rr:unpublished` object is subject to
...
The `rr:published` Motivation is not just the inverse of `rr:unpublished`. It is a full
...
the original is updated and the private `_rerum` metadata is updated with the date
...
sometimes undermine machine obediance.